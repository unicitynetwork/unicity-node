...
TEST_CASE("RPC Commands: Error Handling", "[rpc][integration][errors]") {
    ...
    SECTION("Invalid JSON returns error") {
        // This tests the JSON parsing in HandleClient
        // Note: We can't easily inject malformed JSON through RPCClient
        // since it constructs valid JSON. This would need a raw socket test.
    }
...
TEST_CASE("RPC: Rate Limiting", "[rpc][integration][ratelimit]") {
    ...
    SECTION("Exceeding concurrent request limit returns error") {
        ...
        REQUIRE((success_count + busy_errors) > 0);
    }
}
