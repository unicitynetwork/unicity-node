...
TEST_CASE("Handshake Attack - Half-open connection timeout", "[adversarial][handshake][timeout]") {
    ...
    // Phase 1: Initiate connection (VERSION sent) - attacker scoped to ensure cleanup
    {
        NodeSimulator attacker(2, &network);
        ...
        // Attacker goes out of scope here, ensuring disconnect
    }

    // Process cleanup after attacker destruction
