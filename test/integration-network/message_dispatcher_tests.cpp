...
TEST_CASE("MessageDispatcher - Thread safety", "[network][message_dispatcher]") {
  ...
  SECTION("Concurrent registration and dispatch") {
    ...
    // Some dispatches may succeed (timing-dependent)
    // Race: registration may complete before or after corresponding dispatch.
    // Valid range: 0 (all dispatches raced ahead) to 10 (all registrations completed first)
    REQUIRE(dispatch_success >= 0);
    REQUIRE(dispatch_success <= 10);
  }
}
