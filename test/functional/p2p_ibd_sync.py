#!/usr/bin/env python3
"""Test P2P Initial Block Download (IBD) sync.

Tests that a fresh node can sync headers from a peer over P2P.

Scenario:
1. Start node0 with a pre-built chain (12000 blocks)
2. Start node1 (fresh, at genesis)
3. Connect node1 to node0
4. Verify node1 syncs all headers and matches node0's tip

Note: This test verifies P2P sync functionality. For protocol-level
batching tests (MAX_HEADERS_SIZE=80000), see p2p_batching.py which
uses a larger chain (200k+ blocks).
"""
...
def main():
    """Run the test."""
    print("Starting P2P IBD sync test...")
    print("This test verifies that a fresh node can sync from a peer")
    print()
    ...
    try:
        ...
        print(f"\nWaiting for IBD to complete ({total} headers)...")
        print()
        ...
        print(f"\n✓ Test passed! P2P IBD sync works correctly:")
        print(f"  Node1 synced all {info0['blocks']} headers from node0")
