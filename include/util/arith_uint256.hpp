...
class base_uint {
  // Allow different bit-width instantiations to access each other's pn[] for ::from()
...
public:
  ...
  // Convert from another base_uint of different bit width
  // Copies min(WIDTH, B.WIDTH) 32-bit words, zero-extends or truncates as needed
  template <unsigned int B>
  static base_uint from(const base_uint<B>& b) {
    base_uint ret;
    constexpr int OTHER_WIDTH = B / 32;
    for (int i = 0; i < std::min(WIDTH, OTHER_WIDTH); i++)
      ret.pn[i] = b.pn[i];
    return ret;
  }
...
// 512-bit unsigned big integer support (matches alpha-release)
